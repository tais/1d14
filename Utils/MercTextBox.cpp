...
//	pSrcBuf = ( UINT16* )LockVideoSurface( gPopUpTextBox->uiSourceBufferIndex, &uiSrcPitchBYTES);
	...
	// Opt-in diagnostic (set JA2_BOX_DEBUG): after the panel blit, sample the box
	// surface centre AND the FRAME_BUFFER dest centre. If they match, the blit
	// landed the panel into the frame buffer and the fault is downstream (upload/
	// present). If they differ, the surface->frame blit is a runtime no-op. Also
	// dumps the box flags/index/geometry and the blit return. Zero cost unless set.
	if ( uiBuffer == FRAME_BUFFER && getenv("JA2_BOX_DEBUG") )
	{
		UINT32 sp = 0, dp = 0;
		...
		UINT16 dstPx = db ? db[ ( sDestY + cy ) * ( dp >> 1 ) + ( sDestX + cx ) ] : 0xDEAD;
		UnLockVideoSurface( gPopUpTextBox->uiSourceBufferIndex );
		UnLockVideoSurface( FRAME_BUFFER );
		FILE *f = fopen( "box_debug.log", "a" );
		if ( f )
		{
			fprintf( f, "bltOK=%d flags=%x idx=%u wh=%dx%d dst=%d,%d srcPx=%04x dstPx=%04x %s\n",
				(int)fBltOK, gPopUpTextBox->uiFlags, gPopUpTextBox->uiSourceBufferIndex,
				gPopUpTextBox->sWidth, gPopUpTextBox->sHeight, sDestX, sDestY, srcPx, dstPx,
				( srcPx == dstPx ) ? "MATCH(blit landed)" : "DIFF(blit no-op)" );
			fclose( f );
		}
	}
